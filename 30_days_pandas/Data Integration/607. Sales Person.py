...
he ID of the salesperson, the date of the order, and the amount paid.
...
+------+
Explanation: 
According to orders 3 and 4 in the Orders table, it is easy to tell that only salesperson John and Pam have 
...
def sales_person(sales_person: pd.DataFrame, company: pd.DataFrame, orders: pd.DataFrame) -> pd.DataFrame:
    # Get com_id that is RED
    com_ids = company["com_id"][company["name"] == "RED"]
    # Get sale_id
    sale_ids = orders["sales_id"][orders["com_id"].isin(com_ids)]
    # Get name
    return sales_person[["name"]][~sales_person["sales_id"].isin(sale_ids)]